...
TARGET_FOLDER="3D_golf_ball_vanilla_CholmodSupernodalLLT_valgrind_6"
BIN_FILE="/u/1/chenyang/LearnPolyFEM/polyfem/build.valgrind_multithreads_1/PolyFEM_bin"
JSON_FILE="/u/1/chenyang/benchmark_data/larger_matrix_exp/contact/examples/3D/golf-ball.json"

# Absolute path to target folder
target_folder="/mnt/hdd1/chenyang/benchmark_data/larger_matrix_exp/new_mat_bin/${TARGET_FOLDER}"
...
# Valgrind log files
valgrind_log="$target_folder/valgrind_helgrind.log"
...
# Run with Valgrind Helgrind (race condition detector)
...
echo "Valgrind Helgrind log saved to: $valgrind_log"
echo "First 100 lines of Helgrind output:"
head -100 "$valgrind_log"
